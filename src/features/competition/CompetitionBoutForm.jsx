...
export default function CompetitionBoutForm({ onSave, compBouts = [] }) {
  ...
  // Shared, user-editable action descriptions
  ...
  return (
    <div className="rounded-xl border border-slate-200 p-3">
      ...
      {rows.map((r) => (
        <div key={r.key} className="grid grid-cols-3 gap-3 items-start">
          <div className="text-left">
            <div className="font-semibold">{r.label}</div>
            <input
              type="text"
              placeholder={`Describe ‘${r.label}’…`}
              value={actionDesc[r.key]}
              onChange={(e)=>upDesc(r.key, e.target.value)}
  ...
  const save = () => {
    const row = { ...form, ...totals, CES, createdAt: new Date().toISOString(), type: "competition" };
    onSave(row);
    ...
    <div className="rounded-2xl border border-slate-200 bg-white p-4 space-y-5">
      ...
      <div className="grid sm:grid-cols-6 gap-3">
        ...
        <label className="text-sm">
          ...
          <input type="text" value={form.nation}
      ...
      <div className="rounded-xl border border-slate-200 p-3">
        ...
        <p className="text-xs text-slate-600 mb-3">Download all competition bouts with rounds, segments, and CES.</p>
        <button type="button" onClick={exportCompetition}
